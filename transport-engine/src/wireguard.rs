...
/// WireGuard transport delegating connect/disconnect to a shared VPN backend.
...
#[async_trait]
impl TransportBackend for WireGuardTransport {
    ...
    async fn start(&self, ctx: &TransportContext) -> Result<()> {
        let profile = ctx.vpn_profile.as_ref().ok_or_else(|| {
            WireSentinelError::Other("wireguard transport requires vpn_profile".into())
        })?;

        *self.state.write() = TransportState::Starting;
        info!(name = %ctx.name, profile = %profile.name, "starting wireguard transport");
    ...
    async fn health_check(&self) -> TransportHealth {
        ...
        TransportHealth {
            ...
            message: if connected {
                Some("wireguard connected".into())
            } else {
                Some("wireguard disconnected".into())
            },
        }
    }
}
