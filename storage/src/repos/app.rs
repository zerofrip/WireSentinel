...
use shared_types::{AppExitConfig, AppRecord, WireSentinelError};
use sqlx::SqlitePool;
...
const APP_SELECT: &str = "SELECT app_id, display_name, exe_path, publisher, sha256, icon_path, first_seen, last_seen, default_route_json, exit_config_json FROM apps";
...
fn row_to_app(
    ...
    exit_config_json: Option<String>,
) -> Result<AppRecord> {
    ...
    let exit_config = exit_config_json
        .as_deref()
        .filter(|s| !s.is_empty())
        .map(serde_json::from_str)
        .transpose()
...
impl AppRepository for SqliteAppRepository {
    async fn find_by_id(&self, id: Uuid) -> Result<Option<AppRecord>> {
        let row = sqlx::query_as::<_, AppRow>(&format!("{APP_SELECT} WHERE app_id = ?"))
            .bind(id.to_string())
    ...
    async fn find_by_exe_path(&self, path: &Path) -> Result<Option<AppRecord>> {
        ...
        let row = sqlx::query_as::<_, AppRow>(&format!("{APP_SELECT} WHERE exe_path = ?"))
            .bind(path_str)
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| WireSentinelError::Config(e.to_string()))?;
    ...
    async fn find_by_sha256(&self, sha256: &str) -> Result<Option<AppRecord>> {
        let row = sqlx::query_as::<_, AppRow>(&format!("{APP_SELECT} WHERE sha256 = ?"))
            .bind(sha256)
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| WireSentinelError::Config(e.to_string()))?;
    ...
    async fn upsert(&self, app: &AppRecord) -> Result<()> {
        ...
        sqlx::query(
            r#"
            INSERT INTO apps (app_id, display_name, exe_path, publisher, sha256, icon_path, first_seen, last_seen, default_route_json, exit_config_json)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(exe_path) DO UPDATE SET
                ...
                exit_config_json = excluded.exit_config_json
            "#,
        )
    ...
    async fn list(&self, filter: AppFilter) -> Result<Vec<AppRecord>> {
        ...
        let rows = if let Some(search) = filter.search {
            ...
            sqlx::query_as::<_, AppRow>(&format!(
                "{APP_SELECT} WHERE display_name LIKE ? OR exe_path LIKE ? ORDER BY last_seen DESC LIMIT ?"
            ))
            .bind(&pattern)
            .bind(&pattern)
            .bind(limit)
            ...
            sqlx::query_as::<_, AppRow>(&format!(
                "{APP_SELECT} ORDER BY last_seen DESC LIMIT ?"
            ))
            .bind(limit)
            .fetch_all(&self.pool)
            .await
        ...
        rows.into_iter()
            .map(|r| row_to_app(r.0, r.1, r.2, r.3, r.4, r.5, r.6, r.7, r.8, r.9))
            .collect()
    }
